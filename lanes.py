...
#pyplot is the subpackage of library matplotlib
#Using matplotlib to mark region of interest
...
def display_lines(image,lines):
    ...
    if lines is not None:
        for line in lines:
            #print(parameters)
            x1,y1,x2,y2 = line.reshape(4)
            cv2.line(line_image,(x1,y1),(x2,y2),(255,0,0),10) #b-255,r-0,g-0
    return line_image
...
#Larger the bin and smaller bins precision becomes less
...
combo_image = cv2.addWeighted(lane_image,0.8,line_image,1,1)#0.8 multiplication decressing pixel intensity making it more darker

#Display image
#pyplot contains the function imshow and show
#Before using matplotlib
cv2.imshow('result',combo_image)
cv2.waitKey(0)

#After using matplotlib
#plt.imshow(canny)
#plt.imshow(region_of_interest(cropped_image))
#plt.show()
